...
// Client 半：审查按钮 + 审查弹窗（多模型互审/历史回看）+ 引用胶囊 + 偏好记忆(Host 持久)
// 重新部署：将本文件内容作为 cordis_define 的 code.client 传入（与 host.js 配套）
...
return {
  ...
  apply(ctx) {
    ...
    // Preferences persisted on the Host (survive page refreshes; reset only with a process restart).
    ...
    // ---- Quote chips shown above the composer ----
    let quotes = []
    let quoteSeq = 0
    ...
    const savePrefs = () => {
      host.call('prefs-set', { prefs }).catch(() => {})
    }
    ...
    host.call('prefs-get').then((r) => {
      if (r && r.prefs) {
        prefs = Object.assign({ modelKeys: null, scope: 'current' }, r.prefs)
      }
      return host.call('review-options')
    }).then((opts) => {
    ...
    function ReviewOverlay() {
      ...
      const run = async () => {
        ...
        for (const sel of state.selectedModels) {
          try {
            const start = await host.call('review-start', {
        ...
        const poll = () => {
          for (const j of jobs) {
            ...
            host.call('review-poll', { jobId: j.jobId }).then((res) => {
      ...
      const cancel = () => {
        ...
        state.currentJobIds.forEach((jobId) => {
          host.call('review-cancel', { jobId }).catch(() => {})
        })
...
.krv-overlay{position:fixed;inset:0;z-index:1000;background:rgba(0,0,0,.55);display:flex;align-items:center;justify-content:center}
.krv-box{width:min(920px,94vw);max-height:88vh;display:flex;flex-direction:column;background:var(--dsw-alias-bg-overlay);border:1px solid var(--dsw-alias-border-l1);border-radius:10px;box-shadow:0 12px 40px rgba(0,0,0,.35);color:var(--dsw-alias-label-primary);font-size:13px}
.krv-head{display:flex;align-items:center;justify-content:space-between;padding:10px 14px;border-bottom:1px solid var(--dsw-alias-border-l1);font-weight:600}
...
.krv-md{margin:0;padding:12px 14px;max-height:50vh;overflow:auto;background:var(--dsw-alias-bg-layer-1);font-size:13.5px;line-height:1.65;color:var(--dsw-alias-label-primary)}
...
.krv-cblock{background:var(--dsw-alias-bg-layer-2);border:1px solid var(--dsw-alias-border-l1);border-radius:8px;padding:10px 12px;overflow:auto;max-height:340px;font-size:12px;line-height:1.55;font-family:ui-monospace,SFMono-Regular,Menlo,Consolas,monospace;white-space:pre;margin:8px 0}
