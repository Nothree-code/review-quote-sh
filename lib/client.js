...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		const apply = (ctx) => {
			...
			// 会话节点获取——同时兼容两代 DSH 的 seat：
			//   · 新版（dsh 0.1.2+）：props.useChat → ChatSnapshot.legacy.nodes
			//   · 旧版：props.useSession → session snapshot 的 nodes
			// 两个 hook 都必须以固定顺序调用（React hook 规则），只对「结果」做条件选择；
			// 任一 seat 缺失或返回非数组时降级为空数组，绝不让 slot 渲染抛错。
			const conversationNodesOf = (props) => {
				let chatNodes;
				const useChat = props && props.useChat;
				if (typeof useChat === 'function') {
					try {
						const slice = useChat((s) => ((s !== null && typeof s === 'object') ? s.legacy : undefined));
						chatNodes = (slice !== null && typeof slice === 'object') ? slice.nodes : undefined;
					} catch (e) { /* seat absent */ }
				}
				let sessionNodes;
				...
				if (typeof useSession === 'function') {
					try {
						sessionNodes = useSession((s) => ((s !== null && typeof s === 'object') ? s.nodes : undefined));
					} catch (e) { /* seat absent */ }
				}
			...
			function ReviewButton(props) {
				...
				const node = nodes.find((n) => n && n.kind === 'assistant' && n.messageId === props.messageId);
				const [, force] = react.useState(0);
				react.useEffect(() => subscribe(() => force((x) => x + 1)), []);
				const messages = nodes.filter((n) => n && (n.kind === 'assistant' || n.kind === 'user' || n.kind === 'steering') && nodeText(n).trim());
			...
			function QuoteButton(props) {
				...
				for (let i = nodeIdx - 1; i >= 0; i -= 1) {
					const cand = nodes[i];
					if (cand && (cand.kind === 'user' || cand.kind === 'steering')) { prevUser = cand; break; }
				}
