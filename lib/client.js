			...
			function ReviewOverlay() {
				...
				const summarize = async () => {
					...
					const doneReports = reportKeys
						.filter((k) => state.reports[k].done && state.reports[k].text && !state.reports[k].error)
						.map((k) => {
							const meta = k.split('/');
							const opt = state.options.find((o) => o.provider === meta[0] && o.model === meta[1]);
							return { label: (opt && opt.name) || k, text: state.reports[k].text };
						});
					if (doneReports.length < 2) return;
					const sumModel = state.options.find((o) => o.model === 'kimi-k3')
						|| state.options.find((o) => o.model === 'deepseek-v4-pro')
						|| state.selectedModels[0]
						|| null;
					setState({ summarizing: true, summary: '', summaryError: '', summaryModel: sumModel ? sumModel.name : '默认' });
					try {
						const r = await hostCall('summarize', {
							reports: doneReports,
							model: sumModel ? { provider: sumModel.provider, model: sumModel.model } : null,
						});
...
.krv-sum{font-size:12.5px;padding:5px 16px;border-radius:6px;border:1px solid var(--dsw-alias-state-warn-primary);background:transparent;color:var(--dsw-alias-state-warn-primary);cursor:pointer}
.krv-sum:hover:not(:disabled){background:var(--dsw-alias-bg-layer-2)}
.krv-sum:disabled{opacity:.5;cursor:default}
